...
| `check_arch_canon.py` | 检查 `docs/architecture` 的图 / 文 / 口径表是否**三处同口径**，以及图的节点包含关系是否成立（对应 README 的"三处同改"硬规则） | `python tools/check_arch_canon.py` |
| `check_arch_style.py` | 检查 `docs/architecture` 的文风是否失衡：概括句 >45 字、长句（>90 字）占比 >1/3、单行 ≥8 个标识符、连续无标点 >60 字、图上标签缺两层写法 | `python tools/check_arch_style.py` |
