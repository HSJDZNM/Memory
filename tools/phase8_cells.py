"""Phase 8 学习手册的单元内容（被 tools/build_learning_notebook.py 引用）。

单独成文件的原因与 Phase 6 / Phase 7 一样：build_learning_notebook.py 已经六千多行，
把某一阶段的讲解再塞进去会让"生成器"和"内容"混在一起。**改动手册内容 = 改这个文件**，
然后运行：
...
RunReport（status / failure.code / steps），退出码只出现在 python -m orchestration.cli
里，而不在这份手册里。

所有临时产物写在 .tmp/learning-phase-8/ 下，且每轮先删再建——手册会被跑两遍，
第二次不能因为"文件已存在"失败；每个场景又有自己的子目录，所以互不覆盖。

手册里**不联网、不起端口、不调用 LLM**：第 4 节起全部走
orchestration.runtime.build_assembly 注入的 ScriptedPolicyClient +
RecordingToolRunner（阶段计划 §2 的"先用 fake Policy Client"），
只有第 6 节会真的构造 LangGraph 引擎（并用逐字段比对证明它与参考引擎等价）。
"""
...
__all__ = ["PHASE_8_CELLS", "check_phase_8_structure"]
...
PHASE_8_CELLS: list[tuple[str, str]] = [
    _markdown("""
...
def check_phase_8_structure(namespace: dict[str, Any]) -> list[str]:
    """核对手册里写过的关键数值、键名与结论（生成期断言）。"""

    problems: list[str] = []

    if namespace.get('state_revision_trace') != (0, 1, 2):
        problems.append('revision 没有随每次修改 +1: ' + repr(namespace.get('state_revision_trace')))
    if namespace.get('state_digest_stable') is not True:
        problems.append('相同输入没有给出相同的状态摘要')
    if namespace.get('state_digest_changes') is not True:
        problems.append('改了字段之后状态摘要没有变化：摘要不可信')

    expected_refusals = {
        'unknown_field': 'tools -> Extra inputs are not permitted',
        'unknown_version': 'state_schema_version -> 未知的编排状态版本：9.9',
        'absolute_path': 'path -> 状态里不许出现绝对路径',
        'parent_path': 'path -> 路径不得包含空段、. 或 ..',
        'backslash': 'path -> 状态里的路径统一用 / 分隔',
    }
    if namespace.get('state_refusals') != expected_refusals:
        problems.append('状态的拒绝方式与文档不一致: ' + repr(namespace.get('state_refusals')))

    expected_limits = {
        'repair_rounds': ('repair_rounds', 'max_repair_rounds', 'limit_repair_rounds'),
        'tool_calls': ('tool_calls', 'max_tool_calls', 'limit_tool_calls'),
        'node_runs': ('node_runs', 'max_node_runs', 'limit_node_runs'),
        'tokens': ('tokens', 'max_tokens', 'limit_tokens'),
        'cost_units': ('cost_units', 'max_cost_units', 'limit_cost'),
        'elapsed_ms': ('elapsed_ms', 'max_elapsed_ms', 'limit_wall_clock'),
    }
    if namespace.get('limit_codes') != expected_limits:
        problems.append('上限表与文档不一致: ' + repr(namespace.get('limit_codes')))

    exceeded = namespace.get('limit_exceeded')
    if not exceeded or exceeded[0] != 'limit_repair_rounds' or exceeded[1] != 'needs_human':
        problems.append('击穿 repair 轮次上限的结果与文档不一致: ' + repr(exceeded))

    table = namespace.get('failure_status_table') or {}
    if len(table) < 20:
        problems.append('失败码 → 终态的映射表太小，可能没有真的读 STATUS_BY_CODE: ' + repr(len(table)))
    unmapped = namespace.get('unmapped_failure_codes')
    if unmapped is None:
        problems.append('没有核对"每个失败码都有终态映射"')
    elif unmapped != ():
        problems.append('失败码没有终态映射（新增失败码必须同时登记 STATUS_BY_CODE）: ' + repr(unmapped))

    if namespace.get('spec_problems') != ():
        problems.append('DEFAULT_SPEC 自检没有通过: ' + repr(namespace.get('spec_problems')))
    if namespace.get('route_label_refused') != 'node_contract_invalid':
        problems.append('未知路由标签没有被拒绝: ' + repr(namespace.get('route_label_refused')))
    if '未知路由标签' not in str(namespace.get('route_label_detail')):
        problems.append('未知路由标签的报错没有说明原因: ' + repr(namespace.get('route_label_detail')))
    if not namespace.get('unknown_target_refused'):
        problems.append('条件分支指向未知节点时没有被拒绝')

    if namespace.get('happy_status') != 'completed':
        problems.append('happy path 的终态不是 completed: ' + repr(namespace.get('happy_status')))
    expected_happy = (
        ('requirement_analysis', 'ok', 'ok'),
        ('policy_retrieval', 'ok', 'ok'),
        ('architecture_planning', 'ok', 'ok'),
        ('implementation', 'ok', 'ok'),
        ('validation', 'pass', 'ok'),
        ('testing', 'pass', 'ok'),
        ('review', 'ok', 'ok'),
    )
    if namespace.get('happy_steps') != expected_happy:
        problems.append('happy path 的决策路径与文档不一致: ' + repr(namespace.get('happy_steps')))
    expected_traces = (
        ('implementation', 'learn-8:implementation:0', 'allow'),
        ('validation', 'learn-8:validation:0', 'allow'),
        ('testing', 'learn-8:testing:0', 'allow'),
    )
    if namespace.get('happy_traces') != expected_traces:
        problems.append('happy path 的平台引用与文档不一致: ' + repr(namespace.get('happy_traces')))
    ...
    if namespace.get('repair_steps') != expected_repair:
        problems.append('失败 → 修复 → 通过的路径与文档不一致: ' + repr(namespace.get('repair_steps')))
    counters = namespace.get('repair_counters') or {}
    if counters.get('repair_rounds') != 1 or counters.get('tool_calls') != 2:
        problems.append('修复场景的计数器与文档不一致: ' + repr(counters))
    if namespace.get('repair_runner_calls') != 2:
        problems.append('修复场景的工具调用次数与文档不一致: ' + repr(namespace.get('repair_runner_calls')))

    trace = namespace.get('checkpoint_stage_trace') or ()
    stages = tuple(item[1] for item in trace)
    if len(trace) < 8:
        problems.append('落盘序列太短，没能证明"checkpoint 存的是下一步": ' + repr(stages))
    if 'testing' not in stages or 'review' not in stages:
        problems.append('落盘序列里没有出现预期阶段: ' + repr(stages))
    if namespace.get('checkpoint_stage') != 'review':
        problems.append('最后一份 checkpoint 的 stage 不是收尾节点: '
                        + repr(namespace.get('checkpoint_stage')))

    if namespace.get('engine_payloads_identical') is not True:
        problems.append('两个引擎的报告不是逐字段相同: '
                        + repr(namespace.get('engine_payload_differences')))
    if namespace.get('engine_names') != {'reference': 'reference', 'langgraph': 'langgraph'}:
        problems.append('引擎自述的名字与文档不一致: ' + repr(namespace.get('engine_names')))
    if tuple(namespace.get('engine_dropped') or ()) != ('engine', 'elapsed_ms', 'checkpoints'):
        problems.append('引擎比对的排除字段与文档不一致: ' + repr(namespace.get('engine_dropped')))

    if namespace.get('interrupted_checkpoint_stage') != 'testing':
        problems.append('中断后的 checkpoint 阶段与文档不一致（应当是下一次要跑的 testing）: '
                        + repr(namespace.get('interrupted_checkpoint_stage')))
    if namespace.get('resume_path_equal') is not True:
        problems.append('恢复后的决策路径与不中断时不一致')
    if namespace.get('resumed_flags') != (True, 'resume'):
        problems.append('恢复标志与文档不一致: ' + repr(namespace.get('resumed_flags')))
    if namespace.get('resume_runner_calls') != 0:
        problems.append('恢复后又发生了一次写入（副作用被重放）: '
                        + repr(namespace.get('resume_runner_calls')))
    if namespace.get('checkpoint_leaks') != ():
        problems.append('checkpoint 里出现了不该有的内容: ' + repr(namespace.get('checkpoint_leaks')))

    if namespace.get('approval_missing_status') != 'needs_human':
        problems.append('没有审批时的终态不是 needs_human: '
                        + repr(namespace.get('approval_missing_status')))
    if namespace.get('approval_missing_code') != 'approval_missing':
        problems.append('没有审批时的失败码不是 approval_missing: '
                        + repr(namespace.get('approval_missing_code')))
    if namespace.get('approval_hash_is_not_idempotency_key') is not True:
        problems.append('审批绑定的哈希与编排层的幂等键是同一个值：绑定口径可能又混了')
    if namespace.get('approval_binding_drift') is not True:
        problems.append('参数改一个字符之后平台口径的 action_hash 没有变化')
    if namespace.get('approval_request_binding') != (True, 'orc.fs.edit'):
        problems.append('通过审批的那次请求没有带上审批文件 / 工具不符: '
                        + repr(namespace.get('approval_request_binding')))
    rows = {row[0]: row for row in (namespace.get('approval_rows') or ())}
    expected_approvals = {
        'missing': ('needs_human', 'approval_missing', 0),
        'expired': ('needs_human', 'approval_expired', 0),
        'subject': ('needs_human', 'approval_subject_mismatch', 0),
        'params': ('needs_human', 'approval_param_mismatch', 0),
        'valid': ('completed', None, 1),
    }
    if set(rows) != set(expected_approvals):
        problems.append('审批对照表的场景与文档不一致: ' + repr(sorted(rows)))
    else:
        for name, expected in expected_approvals.items():
            actual = (rows[name][1], rows[name][2], rows[name][3])
            if actual != expected:
                problems.append('审批场景 ' + name + ' 的结果与文档不一致: '
                                + repr(actual) + '，应当是 ' + repr(expected))

    observed = namespace.get('observed_failures') or ()
    codes = {item[1] for item in observed}
    for required in ('node_contract_invalid', 'limit_repair_rounds', 'approval_missing',
                     'approval_expired', 'approval_subject_mismatch', 'approval_param_mismatch'):
        if required not in codes:
            problems.append('失败关闭总表里缺少文档写过的失败码: ' + required)
    if len(observed) < 6:
        problems.append('失败关闭总表太短: ' + repr(observed))
    terminal_counts = namespace.get('terminal_counts') or {}
    total = namespace.get('failure_codes_total')
    if not isinstance(total, int) or sum(terminal_counts.values()) != total:
        problems.append('终态统计与失败码总数不一致: ' + repr(terminal_counts) + ' vs ' + repr(total))
    return problems
