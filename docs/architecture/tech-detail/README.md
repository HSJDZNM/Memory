...
```powershell
# 只写 .drawio（不需要任何第三方库）
python docs/architecture/tech-detail/build.py

# 同时渲染 .png（需要 Pillow）
python docs/architecture/tech-detail/build.py --png
```
...
& "C:\Program Files\draw.io\draw.io.exe" -x -f png -e -s 2 -b 10 `
    -o docs\architecture\tech-detail\00-技术总览.drawio.png `
    docs\architecture\tech-detail\00-技术总览.drawio
```
