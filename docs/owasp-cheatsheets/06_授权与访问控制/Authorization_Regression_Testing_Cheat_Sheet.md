...
The value of an authorization regression suite is only realized if it prevents vulnerable code from merging. The [OWASP CI/CD Security Cheat Sheet](../13_云原生、容器与基础设施/CI-CD与IaC/CI_CD_Security_Cheat_Sheet.md) describes broader pipeline hardening; the recommendations below focus specifically on authorization gates.
