...
crosswalk: ASVS V9 | PC C6 | Top10 A07
-->
...
Despite these concerns, IdP-initiated SSO remains supported for backward compatibility (notably with SAML 1.1). If it must be enabled, the following steps (in addition to those mentioned above) should help secure this flow:
  ...
  * If the contract of the `RelayState` parameter is a URL, make sure the URL is validated and explicitly on an allowlist. This step will help counter the following attack:
    * [Open Redirect](../03_Web前端与浏览器安全/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.md)
...
In many cases, the method of manually setting the association is akin to [Certificate Pinning](../08_传输层、网络与数据保护/Pinning_Cheat_Sheet.md), which is not ideal. Depending on the IdP and SP software, or various design considerations, this may be unavoidable.
...
###### Keys
ECC Keys can be much smaller while providing more security than RSA keys and the math involved is faster to perform. These are preferred when all parties can use them. ECC keys can be as low as 256 bit and still be secure. RSA keys are more interoperable. The minimum RSA key size should be 2048 bit. At least one major vendor [Microsoft Entra](https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-fed-saml-idp) doesn’t support ECC keys.
###### Signing Algorithms
...
If you are using a Hardware Security Module (HSM), follow the vendor's instructions. This process uses openssl. The example uses an overly generic distinguished name. Your Common Name (CN) should be meaningful and specific.
  1. Generate a Private Key: openssl genrsa -out private.key 2048 or openssl ecparam -genkey -name prime256v1 -out private.pem
...
[v3_ca] basicConstraints = CA:FALSE keyUsage = digitalSignature extendedKeyUsage = 1.3.6.1.5.5.7.3.36
  3. Generate the Self-Signed Certificate: openssl req -x509 -new -nodes -key private.key -sha256 -days 365 -out certificate.crt -config cert.cnf -extensions v3_ca
...
The metadata URL should be protected using TLS where the server certificate comes from a WebPKI CA that is widely trusted and matches the guidance in the [Transport Layer Security Cheat Sheet](../08_传输层、网络与数据保护/Transport_Layer_Security_Cheat_Sheet.md#certificates).
...
### Service Provider (SP) Considerations
  ...
  * [Validate Signatures](SAML_Security_Cheat_Sheet.md#validate-signatures)
...
Just because SAML is a security protocol does not mean that input validation goes away.
  * Ensure that all SAML providers/consumers do proper [input validation](../02_输入验证、注入与文件处理/Input_Validation_Cheat_Sheet.md).
