...
crosswalk: ASVS V15 | PC C2 | Top10 A06 | MASVS MASVS-CODE
-->
...
The cheat sheet is not tools oriented but it contains a [tools](Vulnerable_Dependency_Management_Cheat_Sheet.md#tools) section informing the reader about free and commercial solutions that can be used to detect vulnerable dependencies, depending on the level of support on the technologies at hand
**Note:**
...
If in case the provider doesn't properly cooperate with the researcher, the following results are expected:
  ...
  * Most of the time, if the researcher doesn't receive back a response in 30 days, they go ahead and do a [full disclosure](Vulnerable_Dependency_Management_Cheat_Sheet.md#2-full-disclosure) of the vulnerability.
...
When a security issue is detected, it's possible to decide to accept the risk represented by the security issue. However, this decision must be taken by the [Chief Risk Officer](https://en.wikipedia.org/wiki/Chief_risk_officer) (fallback possible to [Chief Information Security Officer](https://en.wikipedia.org/wiki/Chief_information_security_officer)) of the company based on technical feedback from the development team that have analyzed the issue (see the _[Cases](Vulnerable_Dependency_Management_Cheat_Sheet.md#cases)_ section) as well as the CVEs [CVSS](https://www.first.org/cvss/user-guide) score indicators.
## Cases
...
Prior to running the tests, 2 output paths are possible:
  ...
  * One or several tests failed, several output paths are possible:
    ...
    * The fixed version cannot be adopted at all (_e.g._ the fix only ships in a new major version that breaks the application, or the vulnerable version is pinned by a transitive dependency). Apply [Case 5](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-5), using [Case 2](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-2) as an interim mitigation.
    * Technical incompatibility of the released dependency (_e.g._ require a more recent runtime version) which leads to the following actions: 1. Raise the issue to the provider. 2. Apply [Case 2](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-2) while waiting for the provider's feedback.
...
If the provider has provided nothing about the vulnerability, [Case 3](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-3) can be applied skipping the _step 2_ of this case. We assume here that, at least, the [CVE](https://en.wikipedia.org/wiki/Common_Vulnerabilities_and_Exposures) has been provided.
**Step 2:**
...
After identifying the above 2 points, the team is aware of the type of patching that needs to be taken ([Case 2](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-2) with the protective code) and where to add it.
_Example:_
...
**Step 2:**
Using the information from the full disclosure post or the pentester's exploitation feedback, if the provider collaborates then apply [Case 2](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-2), otherwise apply [Case 3](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-3), and instead of analyzing the CVE information, the team needs to analyze the information from the full disclosure post/pentester's exploitation feedback.
### Case 5
#### Context
...
#### Approach
**Step 1:**
Confirm that the upgrade is really blocked by attempting it on a testing environment as described in [Case 1](Vulnerable_Dependency_Management_Cheat_Sheet.md#case-1). Backporting is cheaper than a major upgrade in the short term and more expensive over the life of the project, so it must be a deliberate choice rather than the first reflex. Record the exact blocker (breaking API, transitive pin, unsupported runtime), because that is the condition to re-test later.
**Step 2:**
