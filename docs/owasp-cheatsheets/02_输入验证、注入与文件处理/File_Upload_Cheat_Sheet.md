...
In short, the following principles should be followed to reach a secure file upload implementation:
  ...
  * **Protect the file upload from[CSRF](../03_Web前端与浏览器安全/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.md) attacks**
...
In conjunction with [content-type validation](File_Upload_Cheat_Sheet.md#content-type-validation), validating the file's signature can be checked and verified against the expected file that should be received.
...
As mentioned in the [Public File Retrieval](File_Upload_Cheat_Sheet.md#public-file-retrieval) section, file content can contain malicious, inappropriate, or illegal data.
