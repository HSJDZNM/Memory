...
crosswalk: ASVS V6 | PC C6 | Top10 A07 | MASVS MASVS-AUTH
-->
...
Users should be permitted to use their email address as a username, provided the email is verified during sign-up. Additionally, they should have the option to choose a username other than an email address. For information on validating email addresses, please visit the [input validation cheat sheet email discussion](../02_输入验证、注入与文件处理/Input_Validation_Cheat_Sheet.md#email-address-validation).
...
See: [Transport Layer Security Cheat Sheet](../08_传输层、网络与数据保护/Transport_Layer_Security_Cheat_Sheet.md)
...
**Overview:** Re-authentication is critical when an account has experienced high-risk activity such as account recovery, password resets, or suspicious behavior patterns. This section outlines when and how to trigger re-authentication to protect users and prevent unauthorized access. For further details, see the [Require Re-authentication for Sensitive Features](Authentication_Cheat_Sheet.md#require-re-authentication-for-sensitive-features) section.
...
#### Implementation Recommendations
  * **Minimize User Friction** Ensure that re-authentication does not disrupt the user experience unnecessarily
  * **Context-Aware Decisions** Make re-authentication decisions based on context (e.g., geolocation, device type, prior patterns)
  * **Secure Session Management** Invalidate sessions after re-authentication and rotate tokens—see the [OWASP Session Management Cheat Sheet](Session_Management_Cheat_Sheet.md)
...
Some applications should use a second factor to check whether a user may perform sensitive operations. For more information, see the [Transaction Authorization Cheat Sheet](../06_授权与访问控制/Transaction_Authorization_Cheat_Sheet.md).
...
Error disclosure can also be used as a discrepancy factor, consult the [error handling cheat sheet](../10_日志、监控与错误处理/Error_Handling_Cheat_Sheet.md) regarding the global handling of different errors in an application.
...
### OAuth 2.0 and 2.1
OAuth is an **authorization** framework for delegated access to APIs. See also: [OAuth 2.0 Cheat Sheet](../07_令牌、联合身份与密码学/OAuth2_Cheat_Sheet.md).
...
**See also:[SAML Security Cheat Sheet](../07_令牌、联合身份与密码学/SAML_Security_Cheat_Sheet.md)**
### FIDO
...
Web applications should not make the job of password managers more difficult than necessary by observing the following recommendations:
  ...
  * Implement a reasonable maximum password length, at least 64 characters, as discussed in the [Implement Proper Password Strength Controls section](Authentication_Cheat_Sheet.md#implement-proper-password-strength-controls).
...
_Note: The process is less stringent with[Multifactor Authentication](Multifactor_Authentication_Cheat_Sheet.md), as proof-of-identity is stronger than relying solely on a password._
### Recommended Process If the User HAS [Multifactor Authentication](Multifactor_Authentication_Cheat_Sheet.md) Enabled
  ...
  4. Request the use of [Multifactor Authentication](Multifactor_Authentication_Cheat_Sheet.md) for identity verification.
