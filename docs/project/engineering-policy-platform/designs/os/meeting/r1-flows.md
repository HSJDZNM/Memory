...
> 依据：`docs/project/engineering-policy-platform/designs/os/meeting/r0-facts.md` 的 **F1–F39 / N1–N6**，只引用、不另造口径。
...
| # | 口径 | 锚点 |
| --- | --- | --- |
| 0.1 | 判定只有一条路径 `policy.engine.evaluate`；本地 / dsh / API / 编排都只是消费者 | F5、F31；`docs/project/architecture/功能清单.md:25-29` |
...
**触发者**：人（台阶 4 是唯一人工步骤）。
**前置状态**：镜像在 `docs/mirrors/<mirror>/` 且带 `manifest.json`（F14）。
**注意**：台阶 1–7 与 Phase 0–8 是三套独立编号，图上另有门禁 ①②③（F6、术语与口径 §2）。

| 台阶 | 动作（命令 / 数据） | 标记 | 结果 / 门禁 | 谁负责判定 |
| --- | --- | --- | --- | --- |
| 1 镜像与哈希登记 | `docs/mirrors/<mirror>/**` + `manifest.json`（6 套镜像） | 【读码】`src/retrieval/corpus.py`；【实测】`verify` 报 `datasets=6 entries=27` | 台阶 1 自身不挂门禁，产物被 ① 校验 | 无人判定；只登记 |
...
- 仍属【未知，需核验】：`docs/project/architecture/使用说明.md` 与既有 designs 实施记录未逐行核对；
