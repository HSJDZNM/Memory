...
| X16 | 既有 designs 文档把 `功能清单.md` 的引用写成 :253/:254/:256，实际在 **:323/:324/:326**（偏移约 70 行） | grep `docs/project/architecture/功能清单.md` 实测 | 【实测】 |
...
- **反例**：审计页写"本页可证明未被改过"。
- **后果**：把"删尾/整链重写发现不了"的边界（`docs/project/architecture/功能清单.md:324`）粉饰成安全承诺（F25；`AGENTS.md:151` 约束 16）。
...
- **挡住？** **没有**。实测：真实位置是 `docs/project/architecture/功能清单.md:323`（LLM/MCP/向量库未引入）、`:324`（摘要链边界）、`:326`（审批无签名与名册），
