...
| `diagrams/build.py` / `notebooks/build_notebooks.py`（在 `docs/project/architecture/tech-detail/`，不是 `tools/`） | 分别生成 tech-detail 十张图的 `.drawio`/`.png` 与十份**讲解 notebook**（`notebooks/nb_cells/` 是内容源）；notebook 由生成器逐单元真实执行（从两个工作目录各跑一遍），`--check` 是 CI 门禁 |
