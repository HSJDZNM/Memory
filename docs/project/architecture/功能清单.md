...
| 操作平台 / 前端界面 | **没有做**：仓库至今没有任何前端资源（`git ls-files` 里没有 html/js/ts）；"文档 → 规则"的可视化操作仍停留在**提案**阶段，见 [文档转规则操作平台-可行性评估](../engineering-policy-platform/designs/文档转规则操作平台-可行性评估.md)——其中记录了"今天只能同源""新建规则文件绕过审批""HTTP 面漂移静默"等实测结论 |
