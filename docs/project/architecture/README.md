...
| [规则转化覆盖报告.md](规则转化覆盖报告.md) | **转化覆盖报告**：把 `docs/mirrors/<mirror>/**` 的 284 篇镜像文档逐篇走查一遍的结果——17 篇产出 38 条规则、267 篇写明为什么停在 Guidance、被拒绝但值得记下来的候选、走查中翻出的真缺陷，以及 §10 的二次分析（转化率的四个口径、已转化与未转化两方的特征、值得转化的候选、需要收窄或降级的规则——含 192 个真实文件的运行时实测） |
