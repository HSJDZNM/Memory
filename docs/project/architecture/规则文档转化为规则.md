...
**仓库里的规则文档不会"自动"变成规则。** `docs/mirrors/<mirror>/**` 下的官方文档首先是**只读的追溯与检索语料**；
转化是一次**显式、可评审的提炼动作**。
...
三层规范模型决定"能不能阻断"。
原始定义在 `docs/project/engineering-policy-platform` 下的 `02-repository-data-sources.md`。
...
(docs/mirrors/<mirror>/**)  →人工/受控提炼→  (knowledge/corpus.yaml)  →明确采纳+可验证测试→  (policies/**/*.yaml)
离线镜像原文                          检索与解释，不阻断              机器可执行，可阻断执行
```

| 层 | 载体 | 谁能读 | 能不能阻断执行 |
| --- | --- | --- | --- |
| Raw Reference | `docs/mirrors/owasp-cheatsheets/`、`docs/mirrors/google-eng-practices/`、`docs/mirrors/gitlab-code-review/`、`docs/mirrors/python-pep-code-style/`、`docs/mirrors/dotnet-design-guidelines/`、`docs/mirrors/dora-capabilities/`（逐字复制上游原文 + `manifest.json`） | 人、`tools/mirror_docs.py`、检索摄取 | **不能**。网页指令、代码与示例不得直接触发工具 |
...
| --- | --- |
| 输入 | 上游官方文档（URL + 抓取时间） |
| 动作 | `tools/mirror_docs.py` 等镜像流水线抓取，逐字写入 `docs/mirrors/<mirror>/**`，并在 `docs/mirrors/<mirror>/manifest.json` 记录每页的 `local_path` / `source_url` / `title` / `sha256` / `bytes` |
...
| 项 | 内容 |
| --- | --- |
| 输入 | `docs/mirrors/<mirror>/manifest.json` |
...
1. **曾经的静默丢字（已修复）**：段落超预算时按行边界拆开，打包器**丢掉了截断标记**——
   实测 `core/continuous-integration.md` 那一行 1253 字，索引后 chunk 是 1200 字、`truncated=0`、
   `original_chars` 为空，run 报告也写 `truncated_chunks=0`；另有 OWASP 的
   `06_授权与访问控制/Authorization_Cheat_Sheet.md` 1218 字长行同因，去空白口径分别丢 44 / 17 个字符。
   这与 R8 的纪律冲突，曾是本台阶唯一的已知违约（`chunker.py` 的 `_prose_pieces`）。
   **2026-09-23 已修**：`CHUNKER_VERSION` 升到 `markdown-sections-2`，`_prose_pieces` 删除，
   `_pack()` 直接透传 `_split_prose` 的截断标记，并先按句读拆开、只有拆不动的单元才截断记账。
   验收：全库 `truncated=0`、42 篇逐篇去空白无损、两条长行跨片逐字可拼回
   （证据见设计报告 `分块策略对比-外部框架与本项目.md` §7，台阶 3 的分块策略实测）。
2. **只按行拆、不按句读拆（已随第 1 条一并解决）**：中文文档常常整行没有换行，单行超预算就会踩到第 1 条，
   而外部三家（LangChain、Chonkie、LlamaIndex）都把中文句号问号写进断句规则。
   现在 `markdown-sections-2` 的 `SENTENCE_END_RE` 覆盖英文 `.?!`（后跟空白）与中文 `。！？；…`，
   切点后还要过字素簇检查：不切开汉字与组合字符，只有再无断点的单元才截断。
3. **守卫的现状**：无损检查器 `blocks_are_preserved` 仍然只在单元测试里调用，索引运行期不校验，
   这一条**没有**随修复消失，是本台阶仍然存在的边界（修复后的逐篇核验由一次性脚本完成）。
   `ChunkDraft` 那条校验（标了截断就必须给 `original_chars`）不再被绕过：
   `tests/unit/test_chunker.py` 新增 4 条测试，守着长行拆分、如实记账与 CJK 断句。
...
source:
  ...
  path: docs/mirrors/python-pep-code-style/pep-257-docstrings/index.md   # 约定：指向本地来源，可选
...
| 溯源（**约定，非强制**，但仓库已执行） | 在 `corpus.yaml` 的 `rule_sources` 登记 `{rule_id, rule_version, dataset, source_path, heading_path}`；索引器把标题路径解析成稳定的 `chunk_id` 写进 `rule_sources` 表。当前已登记 **38 条规则**（索引库实测 **141 行** `rule_sources` 记录，2026-09-26 回写 07 §2.7：143 → 141），覆盖全部由镜像文档提炼的规则。
...
| C33 | **约定（非强制）**：由镜像文档提炼的规则在 `rule_sources` 登记溯源；一旦登记就必须能解析到 chunk | 不登记：**没有任何报错**（代码不强制；仓库从 2026-09 起已把 38 条镜像规则的溯源全部登记）；登记了却解析不到 → 索引 run 失败（`IndexingError`），**不静默** |
...
| 台阶 | 实际内容 |
| --- | --- |
| 台阶 1 | `docs/mirrors/python-pep-code-style/pep-257-docstrings/index.md`（数据集 `python-pep-code-style`，public domain） |
