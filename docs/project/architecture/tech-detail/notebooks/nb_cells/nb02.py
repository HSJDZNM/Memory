...
# -*- coding: utf-8 -*-
"""02-dsh-Hook-内部流程：PreToolUse 九步与失败关闭（内容源，产物由 build_notebooks.py 生成）。"""
from __future__ import annotations

from nb_cells import NotebookSpec, code, markdown

SPEC = NotebookSpec(
    stem="02-dsh-Hook-内部流程",
    title="单技术：dsh Hook 内部流程",
    summary="把 PreToolUse 九步跑一遍：事件映射、工具表白名单、路径范围、平台判定、exit 0/2、审计与 PostToolUse，并证明失败关闭由 Hook 自己保证",
    temp_dir=".tmp/tech-detail/02",
    cells=(
        markdown(
            '''
# 02 dsh Hook 内部流程
