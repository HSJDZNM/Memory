"""Policy API：传输边界，不是第二份业务逻辑：tech-detail 讲解 notebook 的纯 Python 版本。

由 docs/project/architecture/tech-detail/notebooks/build_notebooks.py 生成，内容与同名的
.ipynb 逐字相同（那份里每段代码也是一个单元）。直接运行本文件即可复现全部输出：

    python docs/project/architecture/tech-detail/notebooks/06-Policy-API.py
...
ops_metrics = CLIENT.get("/v1/ops/metrics", headers={"Authorization": "Bearer ops-monitor-token"})
