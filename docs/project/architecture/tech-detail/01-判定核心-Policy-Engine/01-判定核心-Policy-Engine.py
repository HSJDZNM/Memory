"""单技术：Policy Engine 内部流程：tech-detail 讲解 notebook 的纯 Python 版本。

由 docs/project/architecture/tech-detail/build_notebooks.py 生成，内容与同名的
.ipynb 逐字相同（那份里每段代码也是一个单元）。直接运行本文件即可复现全部输出：

    python docs/project/architecture/tech-detail/01-判定核心-Policy-Engine/01-判定核心-Policy-Engine.py
