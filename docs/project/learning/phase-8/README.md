...
```powershell
# 交互式阅读（需要 jupyter）
jupyter lab docs/project/learning/phase-8/walkthrough.ipynb
...
2. 提交前运行 `python tools/check_notebook.py docs/project/learning/phase-8/walkthrough.ipynb`
   与 `python tools/build_learning_notebook.py --check`；
3. 表格一律用生成器注入的 `pad()` 补位（中文占两列，`f"{x:<10}"` 会把表挤歪）；
4. 新增阶段时同步更新 `docs/project/learning/README.md` 的索引。
...
A：`git checkout -- docs/project/learning/phase-8/walkthrough.ipynb`，或重新生成
`python tools/build_learning_notebook.py --phase phase-8`。
