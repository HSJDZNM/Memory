...
| Phase 7 Policy API | [phase-7/](phase-7/) | DTO 与领域模型分离、错误码 → 状态码、租户与令牌边界、预算与超时、幂等台账、本地与经 API 的决策整份相等、readiness 与观测锚定 |
...
- **提交之前**：不要提交带运行痕迹的手册。恢复方式二选一——
  ...
  `git checkout -- docs/project/learning/<phase>/walkthrough.ipynb`（只还原这一个文件）；
- **想彻底避开**：直接运行 `python docs/project/learning/<phase>/walkthrough.py`。它和 notebook
...
1. 新建 `docs/project/learning/<phase>/` 目录，放齐上面四个文件；
...
3. 在本文件顶部的表格里加一行；
4. 提交前运行 `python tools/check_notebook.py docs/project/learning/*/walkthrough.ipynb`。

## 与其他文档的分工

| 文档 | 读者 | 回答什么 |
| --- | --- | --- |
| `docs/project/engineering-policy-platform/` | 设计与实施者 | 每个阶段要做什么、如何验收 |
| `docs/project/learning/<phase>/note.md` | 想搞懂结构的人 | 本阶段有哪些对象、关系如何、边界在哪 |
| `docs/project/learning/<phase>/walkthrough.*` | 想动手的人 | 这一步实际跑起来是什么样、为什么这么设计 |
