...
source_url: "https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/abstractions-abstract-types-and-interfaces/"
role: "指南文档 · Abstractions (abstract types and interfaces)"
why: "上游目录路径：Framework design guidelines > Design for extensibility > Abstractions (abstract types and interfaces)（同级第 5 篇）"
...
An abstraction is a type that describes a contract but does not provide a full implementation of the contract. Abstractions are usually implemented as abstract classes or interfaces, and they come with a well-defined set of reference documentation describing the required semantics of the types implementing the contract. Some of the most important abstractions in the .NET Framework include [Stream](https://learn.microsoft.com/en-us/dotnet/api/system.io.stream), [IEnumerable<T>](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.ienumerable-1), and [Object](https://learn.microsoft.com/en-us/dotnet/api/system.object).
