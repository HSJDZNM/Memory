...
Select-String -Path docs/engineering-policy-platform/designs/console -Recurse -Pattern 'innerHTML|insertAdjacentHTML|outerHTML|document.write'
```
...
| P6 | 引用漂移约 70 行 | X16、A13 | 【复核读码】`docs/architecture/功能清单.md:251-258` 是 `tools/` 脚本表；LLM/MCP/向量库在 `:323`、审计链边界在 `:324`、审批无签名在 `:326` | **成立** |
