...
- **F2** 六层架构（架构图文口径的唯一真相源）：① 消费方/入口、② 接入与协议转换、③ 判定核心、④ 能力层、⑤ 数据与契约、⑥ 证据与门禁。（docs/architecture/README.md:8）
- **F3** 12 项能力清单与主要入口。（docs/architecture/功能清单.md:45-60）
- **F4** 九阶段 Phase 0–8 全部实现完成；未完成项只有两个：第二个真实 Agent 产品验证（Phase 6）、真实模型作者 ChangeAuthor（Phase 8）。（docs/engineering-policy-platform/README.md:45-59、功能清单.md:33-43）
...
- **F6** 台阶 1–7（规则文档 → 可执行规则）与 Phase 0–8 是三套独立编号，对照见 docs/architecture/术语与口径.md §2；图上标 ①②③。
...
- **F26** 已有一份尚未提交（untracked）的操作台提案：docs/engineering-policy-platform/designs/ 下 5 份文档（可行性评估、前端交互逻辑、后端契约与连接证据、评审意见、分块策略对比），以及可点开的静态原型 designs/console/（6 个 HTML + assets/app.js + assets/style.css + 由 build_site.py 生成的 assets/data.js）。（git status 显示 `?? docs/engineering-policy-platform/designs/`）
...
- **N3** 没有写路由、没有 CORS：前端与 API 只能同源（既有设计的实测结论：无 CORS、只能同源）。（docs/engineering-policy-platform/README.md:38-40）
