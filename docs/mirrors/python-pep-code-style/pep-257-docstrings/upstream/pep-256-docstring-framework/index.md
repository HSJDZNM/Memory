...
source_url: "https://peps.python.org/pep-0256/"
section: "pep-257-docstrings/upstream/pep-256-docstring-framework"
role: "PEP 256 - Docstring Processing System Framework"
why: "PEP 257 所引 Docutils 系统的框架规范"
...
Python lends itself to inline documentation. With its built-in docstring syntax, a limited form of [Literate Programming](http://www.literateprogramming.com/) is easy to do in Python. However, there are no satisfactory standard tools for extracting and processing Python docstrings. The lack of a standard toolset is a significant gap in Python’s infrastructure; this PEP aims to fill the gap.
...
There are many aspects to docstring processing. The “Docstring PEPs” have broken up the issues in order to deal with each of them in isolation, or as close as possible. The individual aspects and associated PEPs are as follows:
  ...
  * Docstring syntax. [PEP 287](https://peps.python.org/pep-0287/ "PEP 287 – reStructuredText Docstring Format"), “reStructuredText Docstring Format”, proposes a syntax for Python docstrings, PEPs, and other uses.
  * Docstring semantics consist of at least two aspects:
    * Conventions: the high-level structure of docstrings. Dealt with in [PEP 257](../../index.md "PEP 257 – Docstring Conventions"), “Docstring Conventions”.
    * Methodology: rules for the informational content of docstrings. Not addressed.
  * Processing mechanisms. This PEP ([PEP 256](index.md "PEP 256 – Docstring Processing System Framework")) outlines the high-level issues and specification of an abstract docstring processing system (DPS). [PEP 258](../pep-258-docutils-design/index.md "PEP 258 – Docutils Design Specification"), “Docutils Design Specification”, is an overview of the design and implementation of one DPS under development.
  * Output styles: developers want the documentation generated from their source code to look good, and there are many different ideas about what that means. [PEP 258](../pep-258-docutils-design/index.md "PEP 258 – Docutils Design Specification") touches on “Stylist Transforms”. This aspect of docstring processing has yet to be fully explored.

By separating out the issues, we can form consensus more easily (smaller fights ;-), and accept divergence more readily.

## Rationale

There are standard inline documentation systems for some other languages. For example, Perl has [POD](http://www.perldoc.com/perl5.6/pod/perlpod.html) (“Plain Old Documentation”) and Java has [Javadoc](http://java.sun.com/j2se/javadoc/), but neither of these mesh with the Pythonic way. POD syntax is very explicit, but takes after Perl in terms of readability. Javadoc is HTML-centric; except for “`@field`” tags, raw HTML is used for markup. There are also general tools such as [Autoduck](http://www.helpmaster.com/hlp-developmentaids-autoduck.htm) and [Web](http://www-cs-faculty.stanford.edu/~knuth/cweb.html) (Tangle & Weave), useful for multiple languages.

There have been many attempts to write auto-documentation systems for Python (not an exhaustive list):

  * Marc-Andre Lemburg’s [doc.py](http://www.egenix.com/files/python/SoftwareDescriptions.html#doc.py)
  * Daniel Larsson’s [pythondoc](http://starship.python.net/crew/danilo/pythondoc/) & [gendoc](http://starship.python.net/crew/danilo/pythondoc/)
  * Doug Hellmann’s [HappyDoc](http://happydoc.sourceforge.net/)
  * Laurence Tratt’s Crystal (no longer available on the web)
  * Ka-Ping Yee’s [pydoc](http://docs.python.org/library/pydoc.html) (pydoc.py is now part of the Python standard library; see below)
  * Tony Ibbs’ [docutils](http://www.tibsnjoan.co.uk/docutils.html) (Tony has donated this name to the [Docutils project](http://docutils.sourceforge.net/))
  * Edward Loper’s [STminus](http://www.cis.upenn.edu/~edloper/pydoc/) formalization and related efforts
...
PyDoc became part of the Python standard library as of release 2.1. It extracts and displays docstrings from within the Python interactive interpreter, from the shell command line, and from a GUI window into a web browser (HTML). Although a very useful tool, PyDoc has several deficiencies, including:
  ...
  * PyDoc extracts docstrings and structural information (class identifiers, method signatures, etc.) from imported module objects. There are security issues involved with importing untrusted code. Also, information from the source is lost when importing, such as comments, “additional docstrings” (string literals in non-docstring contexts; see [PEP 258](../pep-258-docutils-design/index.md "PEP 258 – Docutils Design Specification")), and the order of definitions.

The functionality proposed in this PEP could be added to or used by PyDoc when serving HTML pages. The proposed docstring processing system’s functionality is much more than PyDoc needs in its current form. Either an independent tool will be developed (which PyDoc may or may not use), or PyDoc could be expanded to encompass this functionality and _become_ the docstring processing system (or one such system). That decision is beyond the scope of this PEP.
...
[PEP 257](../../index.md "PEP 257 – Docstring Conventions") documents some of these issues.
...
These issues are applicable to any docstring processing system implementation. [PEP 258](../pep-258-docutils-design/index.md "PEP 258 – Docutils Design Specification") documents these issues.

  3. Docstring processing system implementation.
  4. Input markup specifications: docstring syntax. [PEP 287](https://peps.python.org/pep-0287/ "PEP 287 – reStructuredText Docstring Format") proposes a standard syntax.
