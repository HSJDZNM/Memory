<!--
source: https://cheatsheetseries.owasp.org/cheatsheets/Nextjs_Security_Cheat_Sheet.html
fetched: 2026-09-16
site: OWASP Cheat Sheet Series (https://cheatsheetseries.owasp.org/)
category: 12_编程语言与框架专项/JavaScript与TypeScript
title: Next.js Security Cheat Sheet
...
For browser rendering sinks and React-specific client concerns, use the [XSS Prevention Cheat Sheet](../../02_输入验证、注入与文件处理/Cross_Site_Scripting_Prevention_Cheat_Sheet.md) and [DOM-based XSS Prevention Cheat Sheet](../../02_输入验证、注入与文件处理/DOM_based_XSS_Prevention_Cheat_Sheet.md).
...
Review `next.config.*`, Proxy matchers, and route configuration with the same care as application handlers. Next.js [rewrites act as a URL proxy](https://nextjs.org/docs/app/api-reference/config/next-config-js/rewrites), and matcher values can flow into destinations.
  ...
  * Do not let untrusted captures select the scheme or authority of an external redirect. Validate any interpolated path or query component using the [Unvalidated Redirects and Forwards Cheat Sheet](../../03_Web前端与浏览器安全/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.md).
