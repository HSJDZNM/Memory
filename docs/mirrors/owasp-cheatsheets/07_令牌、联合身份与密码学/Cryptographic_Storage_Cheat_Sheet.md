...
crosswalk: ASVS V11 V13 V14 | PC C8 | Top10 A02 | MASVS MASVS-CRYPTO MASVS-STORAGE
-->

# Cryptographic Storage Cheat Sheet
## Introduction
This article provides a simple model to follow when implementing solutions to protect data at rest.
Passwords should not be stored using reversible encryption - secure password hashing algorithms should be used instead. The [Password Storage Cheat Sheet](../05_身份认证与会话管理/Password_Storage_Cheat_Sheet.md) contains further guidance on storing passwords.
...
This process should begin with considering the [threat model](../01_安全需求、架构与治理/Threat_Modeling_Cheat_Sheet.md) of the application (i.e, who you are trying to protect that data against).
...
## Algorithms
For symmetric encryption **AES** with a key that's at least **128 bits** (ideally **256 bits**) and a secure [mode](Cryptographic_Storage_Cheat_Sheet.md#cipher-modes) should be used as the preferred algorithm.
...
### Key Generation
Keys should be randomly generated using a cryptographically secure function, such as those discussed in the [Secure Random Number Generation](Cryptographic_Storage_Cheat_Sheet.md#secure-random-number-generation) section. Keys **should not** be based on common words or phrases, or on "random" characters generated by mashing the keyboard.
