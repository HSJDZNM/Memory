...
This Cheat Sheet provides guidance on the various areas that need to be considered when working with GraphQL:
  * Apply proper [input validation](../02_输入验证、注入与文件处理/Input_Validation_Cheat_Sheet.md) checks on all incoming data.
  * Expensive queries will lead to [Denial of Service (DoS)](../11_业务逻辑与反自动化/Denial_of_Service_Cheat_Sheet.md), so add checks to limit or prevent queries that are too expensive.
...
See the OWASP Cheat Sheets on [Input Validation](../02_输入验证、注入与文件处理/Input_Validation_Cheat_Sheet.md) and general [injection prevention](../02_输入验证、注入与文件处理/Injection_Prevention_Cheat_Sheet.md) for full details to best perform input validation and prevent injection.
#### General Practices
Validate all incoming data to only allow valid values (i.e. allowlist).
  ...
  * [List allowed characters](../02_输入验证、注入与文件处理/Input_Validation_Cheat_Sheet.md#allow-list-vs-block-list) - don't use a denylist
  ...
  * To properly handle unicode input, use a [single internal character encoding](../02_输入验证、注入与文件处理/Input_Validation_Cheat_Sheet.md#validating-free-form-unicode-text)
  * Gracefully [reject invalid input](../10_日志、监控与错误处理/Error_Handling_Cheat_Sheet.md), being careful not to reveal excessive information about how the API and its validation works.
...
For more information see the below pages:
  ...
  * [OS Command Injection Prevention](../02_输入验证、注入与文件处理/OS_Command_Injection_Defense_Cheat_Sheet.md)
  * [XML Security](../02_输入验证、注入与文件处理/XML_Security_Cheat_Sheet.md) and [XXE Injection Prevention](../02_输入验证、注入与文件处理/XML_External_Entity_Prevention_Cheat_Sheet.md)
...
However, containerization platforms tend to make this task much easier. See the resource limiting section in the [Docker Security Cheat Sheet](../13_云原生、容器与基础设施/容器与编排/Docker_Security_Cheat_Sheet.md#rule-7-limit-resources-memory-cpu-file-descriptors-processes-restarts) for how to prevent DoS when using containers.
### Access Control
To ensure that a GraphQL API has proper access control, do the following:
  * Always validate that the requester is authorized to view or mutate/modify the data they are requesting. This can be done with [RBAC](https://cheatsheetseries.owasp.org/cheatsheets/Access_Control_Cheat_Sheet.html#role-based-access-control-rbac) or other access control mechanisms.
    * This will prevent [IDOR](../06_授权与访问控制/Insecure_Direct_Object_Reference_Prevention_Cheat_Sheet.md) issues, including both [BOLA](https://github.com/OWASP/API-Security/blob/master/2019/en/src/0xa1-broken-object-level-authorization.md) and [BFLA](https://github.com/OWASP/API-Security/blob/master/2019/en/src/0xa5-broken-function-level-authorization.md).
