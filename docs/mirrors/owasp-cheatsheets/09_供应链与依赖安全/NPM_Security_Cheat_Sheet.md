...
```
  ...
  * Prefer immutable, access-controlled registries or vetted mirrors (private registries, Verdaccio with an upstream cache, or [approved mirrors](NPM_Security_Cheat_Sheet.md#use-a-local-npm-proxy)) and enable retention / immutability policies where available.
  ...
  * Verify packages during CI: check signatures or provenance, validate the SBOM, [run SCA and static analysis](NPM_Security_Cheat_Sheet.md#5-audit-for-vulnerabilities-in-open-source-dependencies), and [install from pinned lockfile resolutions](NPM_Security_Cheat_Sheet.md#2-enforce-the-lockfile).
