...
category: 13_云原生、容器与基础设施/容器与编排
title: Docker Security Cheat Sheet
crosswalk: ASVS V13 | Top10 A05
-->
...
More information about this topic can be found at [Docker official documentation](https://docs.docker.com/engine/security/userns-remap/). For additional security, you can also run in rootless mode, which is discussed in [Rule #11](Docker_Security_Cheat_Sheet.md#rule-11---run-docker-in-rootless-mode).
...
[CI/CD pipelines](../CI-CD与IaC/CI_CD_Security_Cheat_Sheet.md) are a crucial part of the software development lifecycle and should include various security checks such as lint checks, static code analysis, and container scanning.
...
While Docker Secrets generally provide a secure way to manage sensitive data in Docker environments, this approach is not recommended for Kubernetes, where secrets are stored in plaintext by default. In Kubernetes, consider using additional security measures such as etcd encryption, or third-party tools. Refer to the [Secrets Management Cheat Sheet](../../07_令牌、联合身份与密码学/Secrets_Management_Cheat_Sheet.md) for more information.
### RULE #13 - Enhance Supply Chain Security
Building on the principles in [Rule #9](Docker_Security_Cheat_Sheet.md#rule-9---integrate-container-scanning-tools-into-your-cicd-pipeline), enhancing supply chain security involves implementing additional measures to secure the entire lifecycle of container images from creation to deployment. Some of the key practices include:
