<!--
source: https://cheatsheetseries.owasp.org/cheatsheets/Zero_Trust_Architecture_Cheat_Sheet.html
fetched: 2026-09-16
...
Every connection must be encrypted and authenticated, whether it's between your office and the cloud, between internal systems, or from home to work. Network location doesn't determine trust level. Use [strong encryption](../../08_传输层、网络与数据保护/Transport_Layer_Security_Cheat_Sheet.md) (TLS 1.3 or better) for everything.
### 3. Access to Resources is Granted on a Per-Session Basis
Don't give permanent access to anything. Each time someone tries to access a resource, evaluate whether they should be allowed. [Sessions](../../05_身份认证与会话管理/Session_Management_Cheat_Sheet.md) should be short-lived and require re-authentication when they expire. No "set it and forget it" access.
...
You need [MFA](../../05_身份认证与会话管理/Multifactor_Authentication_Cheat_Sheet.md) for everyone - employees, contractors, and partners. Here's what works best:
...
Follow comprehensive [authentication best practices](../../05_身份认证与会话管理/Authentication_Cheat_Sheet.md) when implementing user account management.
### Access Controls
Follow these rules for giving people [access](../../06_授权与访问控制/Authorization_Cheat_Sheet.md):
...
Protect your applications with these controls:
  ...
  * **API security gateways** : Authenticate every API call, validate request schemas, and enforce rate limits for microservices communication using [REST security best practices](../../04_API、微服务与Web服务安全/REST_Security_Cheat_Sheet.md)
...
Keep your data safe with these methods:
  ...
  * **Use encryption** : Protect data whether it's stored or moving with proper [cryptographic storage](../../07_令牌、联合身份与密码学/Cryptographic_Storage_Cheat_Sheet.md)
