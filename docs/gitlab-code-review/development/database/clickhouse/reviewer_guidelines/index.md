...
source_url: "https://docs.gitlab.com/development/database/clickhouse/reviewer_guidelines/"
section: "development/database/clickhouse/reviewer_guidelines"
fetched_at: "2026-09-16T07:45:06Z"
---

# ClickHouse reviewer guidelines
This page provides introductory material and guidance for ClickHouse reviewers.
## Scope of a ClickHouse Reviewer’s Work
ClickHouse reviewers are domain experts with experience in the ClickHouse OLAP database. A ClickHouse database review is required whenever application code that interacts with ClickHouse is modified. Examples include:
...
## Resources for ClickHouse Reviewers
  * [ClickHouse within GitLab](https://docs.gitlab.com/development/database/clickhouse/clickhouse_within_gitlab/): overview of ClickHouse usage in GitLab.
  * [GitLab Database Reviewer Guidelines](../../database_reviewer_guidelines/index.md): general principles that also apply to ClickHouse, especially regarding database migrations. ClickHouse follows the same **pre-deployment** and **post-deployment** migration strategy as our relational databases.
...
Sometimes, the `main.sql` file is not updated or committed in a merge request, leading to inconsistencies between the schema built from migrations and the committed schema file. To detect this issue, a CI job (`clickhouse:check-schema`) runs during the **test** stage. This job compares the newly built schema with `main.sql` and fails if discrepancies are found.
...
ClickHouse queries in GitLab can be written in two ways:
  1. **Raw SQL queries**
  2. **QueryBuilder** - an ActiveRecord-like abstraction ([documentation](https://docs.gitlab.com/development/database/clickhouse/clickhouse_within_gitlab/#writing-query-conditions))
...
While ClickHouse can handle large datasets efficiently, we aim to keep query execution under 10 seconds even for complex aggregations. Performance expectations vary based on feature usage and dataset size.
When reviewing a query:
  1. Ask the author to provide the raw SQL if it’s not clearly visible in the code.
  2. Review the table structure (`SHOW CREATE TABLE table_name FORMAT raw`) to understand partitioning and primary keys.
  3. Confirm that query filters align with the table’s primary key or partitioning columns.
...
In the output, look for the `PrimaryKey` section and check the **Granules** ratio. For example: `185/72937` granules mean only a small subset of the table was scanned - ideal for performance.
...
With the **MergeTree** family, the _primary key_ (that is, `ORDER BY`) defines the sort/index, not a uniqueness constraint. Rows with the same primary-key values can coexist. If your ingestion pipeline might produce duplicates or updates, you must handle them at read time (or pick an engine that collapses versions).
###  `MergeTree` engine
