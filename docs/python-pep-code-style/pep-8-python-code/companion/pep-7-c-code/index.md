...
source_url: "https://peps.python.org/pep-0007/"
section: "pep-8-python-code/companion/pep-7-c-code"
role: "PEP 7 - Style Guide for C Code"
why: "PEP 8 开篇显式指向的配套文档（CPython 的 C 代码风格）"
...
This document gives coding conventions for the C code comprising the C implementation of Python. Please see the companion informational PEP describing [style guidelines for Python code](../../index.md "PEP 8 – Style Guide for Python Code").
...
Follow the following standards. For features that aren’t in the relevant standard, use CPython-specific wrappers (for example: `_Py_atomic_store_int32`, `Py_ALWAYS_INLINE`, `Py_ARITHMETIC_RIGHT_SHIFT`; `_Py_ALIGNED_DEF` in public headers). When adding such wrappers, try to make them easy to adjust for unsupported compilers.
  ...
  * Python 3.11 and newer versions use C11 without [optional features](https://en.wikipedia.org/wiki/C11_%28C_standard_revision%29#Optional_features). The public C API should be compatible with C99 and C++.

(As a reminder to any users reading this: this PEP is a _style guide_ ; these rules are there to be broken.)
  ...
  * Python versions before 3.6 used ANSI/ISO standard C (the 1989 version of the standard). This meant, amongst many other things, that all declarations were at the top of a block.
...
## Common C code conventions
  ...
  * All function declarations and definitions must use full prototypes. That is, specify the types of all arguments and use `(void)` to declare functions with no arguments.
...
```
  ...
  * The return statement should _not_ get redundant parentheses:
...
It’s OK to put operators at ends of lines, especially to be consistent with surrounding code. (See [PEP 8](../../index.md#pep8-operator-linebreak) for a longer discussion.)
